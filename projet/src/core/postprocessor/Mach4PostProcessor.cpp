...
std::string Mach4PostProcessor::generate(
    ...
    const std::string&           programName) const
{
    ...
    out << "%" << "\n";
    out << programName << "\n";
    out << "(CAM-ENGINE — Mach3/Mach4)\n";
    out << "(T" << tool.id << " " << tool.name << " D=" << fmt(tool.diameter) << ")\n\n";

    out << "G00 G17 G40 G49 G80 G90\n";
    out << "T" << tool.id << " M06\n";
    out << "G43 H" << tool.id << "\n";
    out << "S" << static_cast<int>(rpm) << " M03\n";
    out << "G04 P1.\n\n";
    ...
    for (const auto& tp : paths) {
        ...
        for (const auto& mv : tp.moves) {
            ...
            case MoveType::ArcCCW:
                out << (mv.type == MoveType::ArcCW ? "G02" : "G03");
